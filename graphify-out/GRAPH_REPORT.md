...
- [[_COMMUNITY_Conversations (8)|Conversations (8)]]
- [[_COMMUNITY_Rbac (3)|Rbac (3)]]
...
- [[_COMMUNITY_Settings (13)|Settings (13)]]
- [[_COMMUNITY_Lib|Lib]]
...
2. `InvgateService` - 88 edges
3. `AppConfigService` - 73 edges
4. `UsersService` - 58 edges
5. `BrokerService` - 57 edges
6. `apiFetch` - 57 edges
7. `TenantGuard` - 47 edges
8. `ContextSourcesService` - 42 edges
9. `RolesGuard` - 40 edges
10. `RoleService` - 39 edges
...
- `handleTabKeyDown()` --references--> `Pantalla /settings (Next.js)`  [INFERRED]
  apps/web/src/app/settings/page.tsx → docs/plan-de-trabajo.md
- `dependencies` --shares_data_with--> `AuthModule`  [INFERRED]
  apps/api/package.json → docs/plan-de-trabajo.md
...
### Community 0 - "Docs (1)"
Cohesion: 0.05
Nodes (78): Abstracción LLM (LlmService, provider-agnostic), requestJson() valida content-type antes de parsear, Constraint: abstracción LLM vía LlmService, Constraint de abstraccion LLM: la logica de negocio nunca llama al SDK del proveedor, LlmProviderFactory / ResolvedProviderConfig, OpenCodeGoProvider (API de sesiones de opencode), OPENCODEGO_AGENT (default plan), OpenCode Go provider (OpenCodeGoProvider) (+70 more)

### Community 1 - "Users (1)"
Cohesion: 0.08
Nodes (14): User.internalPhone Migration, BulkImportUserRowDto, BulkImportUsersDto, CheckAvailabilityQueryDto, CreateUserDto, CreateUserMultiTenantDto, UpdateUserDto, UpdateUserFullDto (+6 more)

### Community 2 - "Rbac (1)"
Cohesion: 0.06
Nodes (22): API TypeScript Build Config (tsconfig.build.json), API Base TypeScript Config (tsconfig.json), permissions, allow, CreatePermissionDto, PermissionEntryDto, ReplacePermissionsDto, CreateRoleDto (+14 more)

### Community 3 - "Llm"
Cohesion: 0.06
Nodes (25): @nestjs/schedule (dependency), BrokerModule, Conector WhatsApp, Cierre automático de conversaciones por inactividad (decisión, 2026-08-10), whatsapp.incoming queue, whatsapp.outgoing queue, FlowModule, FALLBACK_MODELS (+17 more)

### Community 4 - "Components"
Cohesion: 0.06
Nodes (48): DeletableEdge, BaseNode(), CONDITION_OPERATOR_LABELS, ConditionNode, DelayNode, DeviceValidationNode, EndNode, InputNode (+40 more)

### Community 5 - "Whatsapp (1)"
Cohesion: 0.12
Nodes (27): TwilioContentTemplate (table), Cacheo de Content Template por forma de menú (sin el body), ContentSid resolution order (memoria → BD → Twilio), Carrera de creación concurrente de Content Template (Prisma P2002), Deuda técnica: cache de ContentSid en memoria (se pierde al reiniciar), Deuda técnica: plantillas HSM aprobadas no implementadas (ni Meta ni Twilio), Decisión: botones de Twilio vía Content Template cacheado por forma de menú, Decisión: conectores WhatsApp intercambiables (Meta/Twilio), no coexistencia (+19 more)

### Community 6 - "Edit"
Cohesion: 0.08
Nodes (35): apiFetch, Resolución del tenant: JWT { sub, email }, Header X-Tenant-Id, Flow node components (customNodeTypes), load, submit, Fix: sesión caída no redirigía a /login sola, Sesión deslizante (+27 more)
...
### Community 13 - "Conversations (2)"
Cohesion: 0.14
Nodes (21): StoredAttachment, Flow Node Type Catalog (13 tipos), cancelInteraction, ConversationsService.executeLlmQueryExtraction, executeNode, executeSmsNode, executeTransferAgentNode, findStartNodeId (+13 more)

### Community 14 - "Invgate (1)"
Cohesion: 0.13
Nodes (9): Bloqueado: INVGATE_API_KEY era password de portal, no token API, InvgateCatalogEntry, InvgateIncident, InvgateUser, TicketFieldNames, InvgateService, InvgateService.addComment, InvgateService.listSources (+1 more)

### Community 15 - "Flow (1)"
Cohesion: 0.13
Nodes (12): Flow.contextSourceId column (FK), Flow.skillId Column (Migration), AssignTenantsDto, TenantAssignmentDto, UpdateFlowDto, FLOW_CONTEXT_OPTIONS, FLOW_CONTEXT_VALUES, BLANK_VARIANT_NODES (+4 more)
...
Cohesion: 0.12
Nodes (18): Modelo razonador consume maxTokens antes de responder (bug MiniMax M2.x), Canales adicionales, Decisión: botones de Twilio via Content Template cacheado por hash, Decisión: cache de ContentSid en tabla BD + Map en memoria, Decisión: WhatsApp Meta vs Twilio como conectores intercambiables, Decisión: envío saliente de WhatsApp solo type:text, Despliegue, Ejecución real — 🔄 EN PROGRESO (segunda etapa, pedido 2026-08-11) (+10 more)
...
### Community 53 - "Docs (18)"
Cohesion: 0.15
Nodes (16): Flow Execution State Machine, Saneo del payload de ReactFlow antes de guardar, TenantFlow.isStart (flujo de inicio por tenant), MAX_FLOW_STEPS constant, FlowService.applyTenantAssignment, ConversationsService - orquestador core de mensajes, POST /conversations/simulate (deuda de seguridad aceptada), findActiveFlowForTenant: ambigüedad resuelta con isStart (+8 more)

### Community 54 - "Conversations (3)"
Cohesion: 0.20
Nodes (16): ConversationsService.buildOpenTicketsList, ConversationsService.buildTicketDetailText, ConversationsService.refreshInvgateStatus, ConversationsService.resolveInvgateCustomerId, refreshInvgateStatus, resolveInvgateCustomerId, syncTicketToInvgate, InvGate Integration Milestone (+8 more)

### Community 55 - "Docs (19)"
Cohesion: 0.12
...
### Community 57 - "Schedule Calendar (3)"
Cohesion: 0.12
Nodes (9): CalendarEvent, EntryData, localizer, RECURRENCE_LABEL, RECURRENCE_STEP, ROLE_COLOR_PALETTE, RoleOption, TYPE_COLOR (+1 more)

### Community 58 - "Docs (20)"
Cohesion: 0.16
...
### Community 84 - "Docs (32)"
Cohesion: 0.18
Nodes (11): 2FA Mail/SMS, docs/chatbot.md, Orquestador NestJS, PostgreSQL, RabbitMQ broker, RBAC Dinámico, Soporte Omnicanal con IA, Role entity (+3 more)

### Community 85 - "Docs (33)"
Cohesion: 0.20
...
### Community 94 - "Guards"
Cohesion: 0.27
Nodes (6): Decisión: Tenant activo por header X-Tenant-Id, no JWT, Refactor: TenantGuard reemplaza a TenantInterceptor, TenantInterceptor (obsoleto), TENANT_HEADER, bootstrap(), TenantGuard

### Community 95 - "Invgate (5)"
Cohesion: 0.22
...
### Community 97 - "Web (2)"
Cohesion: 0.22
Nodes (7): devDependencies, eslint, @tailwindcss/postcss, @types/node, @types/react, @types/react-big-calendar, typescript
...
- `twilio-whatsapp.service.ts` → `TwilioWhatsAppService.appendInteractiveAsText()`  [AMBIGUOUS]
...
## Knowledge Gaps
- **565 isolated node(s):** `allow`, `$schema`, `collection`, `sourceRoot`, `deleteOutDir` (+560 more)
...
- **What is the exact relationship between `twilio-whatsapp.service.ts` and `TwilioWhatsAppService.appendInteractiveAsText()`?**
