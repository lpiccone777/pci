...
| BE-IG-10 | Corregir `INVGATE_API_USER` en `/settings` en caliente (técnico que al arranque no matcheaba) y crear un ticket | ✅ `resolveCreatorId` ya **no cachea el fallo**: solo guarda el `creatorId` cuando resolvió a un id real, así que corregir el usuario técnico en `/settings` vuelve a sincronizar sin reiniciar. Residual menor: los cachés de `status`/`priority`/`type` siguen sin invalidarse en caliente (no bloquean la creación de tickets); el de `category` ya tiene botón de recarga en `/settings` (BE-IG-18) |
...
| BE-IG-18 | `POST /settings/invgate/categories/refresh` tras crear una categoría en InvGate **después** del arranque | Vacía `categoryChildrenCache` (entero, no solo el `parentId` actual) y repagina: la categoría nueva aparece en `GET /invgate/catalog/categories` en el **mismo proceso**, sin reiniciar la API. Devuelve `{configured, parentId, categories}` — el `parentId` es el **guardado en BD**, no uno que mande el cliente |
| BE-IG-18b | `POST /settings/invgate/categories/refresh` parado en un tenant que no es el de sistema | 403 (`SystemTenantGuard`). Es una acción de administración: exige tenant de sistema + `settings:update`, no alcanza el `flows:read` que habilita el catálogo de solo lectura |
...
| FE-SET-18 | Botón "Recargar categorías" en la tarjeta `INVGATE_CATEGORY_PARENT_ID` (Integración: InvGate) | `POST /settings/invgate/categories/refresh`; muestra cuántas subcategorías leyó, bajo qué id y sus nombres. Si InvGate no está configurado o no hay categoría padre guardada, lo dice en ámbar en vez de mostrar 0 a secas. Si el id tipeado sin guardar difiere del usado, avisa que recargó con el **guardado**. Deshabilitado sin `settings:update` |
