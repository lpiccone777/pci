...
export class TwilioWebhookController {
  ...
  async receive(@Body() payload: TwilioIncomingPayload) {
    ...
    // Mensaje sin texto (solo media, ubicación, etc.): sin pipeline para eso todavía,
    // mismo alcance que WhatsAppWebhookController con los tipos no soportados de Meta.
    if (!from || !body) {
      this.logger.warn(
        `Mensaje de Twilio ignorado (sin From o sin Body de texto soportado; NumMedia=${payload.NumMedia ?? '0'}).`,
      );
      return { status: 'ok' };
    }
    ...
    await this.broker.publish('whatsapp.incoming', {
      ...
      data: { from, body, channel: 'whatsapp' },
      tenantId,
